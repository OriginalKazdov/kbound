"""Mersenne attack with explicit quotient variable.

Encoding:
    raw = a * s + c
    raw = s_next + m * q       (where q is unknown integer quotient)
    0 <= s_next < m
    0 <= q <= a + 1            (tight bound: q max = (a*(m-1) + c)/m = a + (a-1+c)/m ~ a)

This avoids URem and explicit shift-add reduction. Z3 just needs to find
(a, c, s_0, ..., s_K, q_1, ..., q_K) consistent with the constraints.

Theoretical advantage: tightening q's bit-width reduces Z3's search.
"""
...
def attack_mersenne_quotient(out_seq, p_bits, timeout_s=600):
    K = len(out_seq)
    p = p_bits
    M = (1 << p) - 1
    W = 2 * p + 4  # generous BitVec width to avoid overflow
    ...
    for k in range(K):
        # Quotient bound: q < a + 2 (loose upper bound)
        s.add(ULT(q1[k], a1 + 2))
        s.add(ULT(q2[k], a2 + 2))
        # LCG step: a * s_prev + c == s_next + m * q
        s.add(a1 * s1[k] + c1 == s1[k + 1] + M_bv * q1[k])
        s.add(a2 * s2[k] + c2 == s2[k + 1] + M_bv * q2[k])
        # XOR output
...
for p, lcgs, K in configs:
    M = (1 << p) - 1
    lcgs = [(a % M | 1, c % M, s % M) for (a, c, s) in lcgs]
    ...
    timeout = 60 if p <= 11 else (180 if p <= 17 else (300 if p <= 23 else 900))
    res, elapsed, status = attack_mersenne_quotient(observed, p, timeout_s=timeout)
    if res is None:
